...
def get_imgs_from_dir(path_a,list_a,path_b,list_b):
    file_a = open(list_a)
    list_a = []
    for lines in file_a.readlines():
        line = lines.strip('\n')
        line = os.path.join(path_a, line)
        list_a.append(line)
    file_a.close()
...
class ImageFolder_pair(data.Dataset):
    ...
    def __init__(self, rootA, listA, rootB, listB, info_txt, keep_percent=1, transform=None, transform2=None, return_paths=False,
        ...
        imgs_A_1,imgs_B_1 = get_imgs_from_dir(rootA, listA, rootB, listB)
        ...
        c = list(zip(imgsA, cycle(imgsB), codes))
        random.shuffle(c)
...
def get_data_loader_folder_pair(input_folderA, input_listA, input_folderB, input_listB, info_txt, batch_size, train, keep_percent, new_size=None,
    ...
    dataset = ImageFolder_pair(input_folderA, input_listA, input_folderB, input_listB, info_txt, keep_percent=keep_percent, transform=transform,transform2=transform2,get_direct=get_direct,used_domain=used_domain,return_paths=return_paths)
    return dataset
