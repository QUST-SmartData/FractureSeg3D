...
class DCN(object):
    ...
    def _bilinear_interpolate_3D(self, input_feature, z, y, x):
        ...
        # # modified
        # above code should be changed as " index_a0.type(torch.int64).tolist() " ,but very slowly !!!
        # so, I use ' torch.gather ' replace it !!!
        # # get 8 grid values  ([N*D*W*H,C], [N*D*W*H*27])
        value_a0 = torch.gather(input_feature_flat, 0,
                                index_a0.unsqueeze(1).expand(-1, input_feature_flat.size(1)).astype(torch.int64))
        value_b0 = torch.gather(input_feature_flat, 0,
                                index_b0.unsqueeze(1).expand(-1, input_feature_flat.size(1)).astype(torch.int64))
        value_c0 = torch.gather(input_feature_flat, 0,
                                index_c0.unsqueeze(1).expand(-1, input_feature_flat.size(1)).astype(torch.int64))
        value_d0 = torch.gather(input_feature_flat, 0,
                                index_d0.unsqueeze(1).expand(-1, input_feature_flat.size(1)).astype(torch.int64))
        value_a1 = torch.gather(input_feature_flat, 0,
                                index_a1.unsqueeze(1).expand(-1, input_feature_flat.size(1)).astype(torch.int64))
        value_b1 = torch.gather(input_feature_flat, 0,
                                index_b1.unsqueeze(1).expand(-1, input_feature_flat.size(1)).astype(torch.int64))
        value_c1 = torch.gather(input_feature_flat, 0,
                                index_c1.unsqueeze(1).expand(-1, input_feature_flat.size(1)).astype(torch.int64))
        value_d1 = torch.gather(input_feature_flat, 0,
                                index_d1.unsqueeze(1).expand(-1, input_feature_flat.size(1)).astype(
                                    torch.int64))  # [N*KD*KW*KH, C]
