...
class BCEDiceLoss_Cavity(nn.Module):
    """Linear combination of BCE and Dice losses"""

    def __init__(self, alpha, beta, gamma):
        super(BCEDiceLoss_Cavity, self).__init__()
        self.alpha = alpha
        self.bce = nn.BCEWithLogitsLoss()
        self.beta = beta
        self.gamma = gamma
    ...
    def cavity_loss(self, input, target):
    ...
    def forward(self, input, target):
        ...
        return self.alpha * self.bce(input, target) + self.beta * self.dice(input, target) + self.gamma * self.cavity_loss(input, target)
...
def _create_loss(name, loss_config, weight, ignore_index, pos_weight):
    if name == 'BCEWithLogitsLoss':
        ...
        alpha = loss_config.get('alphs', 1.)
        beta = loss_config.get('beta', 1.)
        gamma = loss_config.get('gamma', 0.5)
        return BCEDiceLoss_cldice(alpha, beta, gamma)
